---
name: ghsphere-score
description: Score any GitHub account 0-100 for real contribution value and trustworthiness, detect bot/farmed activity, compare two developers, and discover top developers by language/org/project. Use when asked to vet, rate, or judge a GitHub user, check if an account is real or farmed, or find strong developers. Backed by ghsphere.com's deterministic open-source engine via a public no-auth REST API and MCP server.
license: AGPL-3.0-or-later
---

# ghsphere — GitHub developer value & trust scoring

ghsphere rates any GitHub account from 0 to 100 for real contribution value and
...
```bash
curl -s https://ghsphere.com/api/score/{username}
```

Full evidence payload (metrics, top repos, recent PRs, red flags):

```bash
curl -s -X POST https://ghsphere.com/api/scan \
...
Full API contract: https://ghsphere.com/openapi.json · Agent docs: https://ghsphere.com/llms.txt

## MCP server

Streamable HTTP MCP server at `https://ghsphere.com/mcp` exposing the tools
`score_user`, `scan_user`, `compare_users`, `get_leaderboard`, `search_users`.
Server card: https://ghsphere.com/.well-known/mcp/server-card.json
...
- Engine is open source (AGPL): https://github.com/MartinKayJr/github-roast
