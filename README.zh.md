...
[**🔍 测测 GitHub 成色**](https://ghsphere.com) · [**🏆 发现最佳开发者**](https://ghsphere.com/leaderboard) · [**⭐ 查看源码**](https://github.com/MartinKayJr/github-roast)

</div>

[![ghsphere 中文开发者主页预览](./show_img/usercard_cn.png)](https://ghsphere.com/u/hikariming)
...
ghsphere 不只负责打分，更是一台开发者发现引擎。你可以通过排行榜和公开开发者主页，找到真正活跃的开源贡献者、同技术栈伙伴、潜在合作者，也能看看那些与你旗鼓相当、值得较量的「对家」——同时让对的人发现你。

[![ghsphere 开发者排行榜](./show_img/leaderboard.png)](https://ghsphere.com/leaderboard)
...
[![ghsphere 评分徽章](https://ghsphere.com/api/badge/hikariming)](https://ghsphere.com/u/hikariming)

<a href="https://ghsphere.com/u/hikariming">
  <picture>
    ...
    <img alt="hikariming 的 ghsphere 开发者卡片" src="https://ghsphere.com/api/card/hikariming?theme=light" width="720">
  </picture>
</a>
...
## ghsphere API、MCP 服务器与 SDK

站点全部能力都可编程调用,完整参考见 **[ghsphere API 文档](https://ghsphere.com/docs)**:

- **REST API** — `GET https://ghsphere.com/api/score/{username}` 拿确定性 0–100 分(无需认证、不经过 LLM);OpenAPI 3.1 规范见 [ghsphere.com/openapi.json](https://ghsphere.com/openapi.json)
- **MCP 服务器** — Streamable HTTP,位于 [ghsphere.com/mcp](https://ghsphere.com/mcp);加进 Claude、Cursor 或任意 MCP 客户端即可在 Agent 内评分和对比 GitHub 账号
- **SDK** — ghsphere JavaScript/TypeScript SDK（npm 包 [`@hikariming/ghfind`](https://www.npmjs.com/package/@hikariming/ghfind)）· ghsphere Python SDK（PyPI 包 [`ghfind`](https://pypi.org/project/ghfind/)）
- **面向 AI Agent** — [ghsphere.com/llms.txt](https://ghsphere.com/llms.txt) 链接所有机器可读入口
...
| `pnpm ghfind` | 面向 agent 的 `ghsphere` CLI,封装网站评分和发现类 API |
...
CLI 命令名仍是 `ghfind`。独立二进制构建为 `./bin/ghfind`, package/bin 元数据也暴露`ghfind`。

默认服务端域名是 `https://ghsphere.com`。本地联调可以覆盖:
...
> **商标声明:** 「ghsphere / 毒舌 GitHub 评分」名称、Logo 及域名**不在本开源协议授权范围内**,版权保留。你可以基于本代码自部署,但请勿使用本项目的名称/品牌冒充官方或制造混淆。
